...
HL.CalculateTimeline = function(timeline)
{
    ...
    HL.LustInfo.useTransformTimeGlobal = timeline[timeline.length - 1].TRANSFORMTIME;
    HL.LustInfo.useLust = false;
    HL.LustInfo.useTransformTime = 60;
    HL.LustInfo.useTransformTimeGlobal = 60;
    ...
    for (var i = 0; i < timeline.length - 1; i++)
    {
        ...
        for (var j = 0; j < loopCount; j++)
        {
            for (var k = 0; k < currentEntrySteps.length; k++)
            {
                ...
                let uptime = parseFloat(currentEntrySteps[k].UPTIME);
                HL.LustInfo.useLust = currentEntrySteps[k].LUST;
                HL.LustInfo.useTransformTime = currentEntrySteps[k].TRANSFORMTIME;
                ...
                switch(type)
                {
                    case "DPS":
                        {
                            if (durationType == "SEC")
                            {
                                while (duration > 0)
                                {
                                    ...
                                    let hpDamageDoneToBoss = HL.GetDamageDone(loopDuration, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, false, multiplier, uptime);
                                    ...
                                    {
                                        let timeRemainingToKill = HL.GetDamageTime(currentBossHP, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, false, multiplier, uptime);
                            ...
                            else if (durationType == "HP")
                            {
                                ...
                                while (hpToDamage > 0)
                                {
                                    ...
                                    if (loopDuration > 0)
                                    {
                                        ...
                                        {
                                            timeToDamageBoss = HL.GetDamageTime(hpToDamage, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, false, multiplier, uptime);
                                    ...
                                    {
                                        let timeToDamageBoss = HL.GetDamageTime(hpToDamage, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, false, multiplier, uptime);
                                        currentBossHP = Math.max(0, currentBossHP - hpToDamage);
                                        currentStrDuration = Math.max(0, currentStrDuration - timeToDamageBoss);
                                        currentFraDuration = Math.max(0, currentFraDuration - timeToDamageBoss);   
                                        finalTime += timeToDamageBoss;
                    ...
                    case "AOE":
                        {
                            if (durationType == "HP")
                            {
                                ...
                                while (hpToDamage > 0)
                                {
                                    ...
                                    if (loopDuration > 0)
                                    {
                                        ...
                                        console.log("Lust Buff duration: " + HL.LustInfo.BUFFTIME);
                                        // get the damage done in this loop
                                        let hpDamageDoneToBoss = HL.GetDamageDone(loopDuration, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, true, multiplier, uptime);
                                        let timeToDamageBoss = 0;

                                        // if this loop duration is longer than needed for the hp we are done
                                        if (hpDamageDoneToBoss > hpToDamage)
                                        {
                                            timeToDamageBoss = HL.GetDamageTime(hpToDamage, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, true, multiplier, uptime);
                                            currentBossHP = Math.max(0, currentBossHP - hpToDamage);
                                            hpToDamage = 0;
                                    ...
                                    {
                                        let timeToDamageBoss = HL.GetDamageTime(hpToDamage, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, true, multiplier, uptime);
                            ...
                            else if (durationType == "SEC")
                            {
                                while (duration > 0)
                                {
                                    ...
                                    if (currentBossHP - hpDamageDoneToBoss < 0)
                                    {
                                        let timeRemainingToKill = HL.GetDamageTime(currentBossHP, currentStrDuration, currentFraDuration, HL.LustInfo.lustTime, HL.LustInfo.lustTransformTime, true, multiplier, uptime);
                                        finalTime += timeRemainingToKill;
                                        currentBossHP = 0;
                                        duration = 0;
                                    }
                    ...
                    case "HP":
                        {
                            let hpToAdd = value;
                            if (HL.dungeonName == 'Palace of the Dead' && (HL.bossIndex == 18 || HL.bossIndex == 8) && HL.LustInfo.BUFFTIME > 0)
                                hpToAdd *= 1.5;                                
                ...
                if (currentBossHP == 0)
                {
                    ...
                    return HL.OutputTimeToKill(finalTime / 60);
                }
            }
        }        
    }        
...
HL.CalculateLust = function(duration)
{
    ...
    if (HL.LustInfo.useLust && HL.LustInfo.lustCount > 0)
    {
        ...
        if (HL.LustInfo.fullLustDuration)
            HL.LustInfo.TRANSFORMTIME = HL.LustInfo.useTransformTime;
    ...
    if (HL.LustInfo.TRANSFORMTIME == 0 && HL.LustInfo.BUFFTIME == 0)
    {
        ...
        if (HL.LustInfo.lustCount > 0 && (!HL.LustInfo.delayLust || HL.LustInfo.lustCount > HL.LustInfo.lustReserveCount))
        {
            HL.LustInfo.lustCount = Math.max(0, HL.LustInfo.lustCount - 1);
            if (HL.LustInfo.fullLustDuration)
                HL.LustInfo.TRANSFORMTIME = HL.LustInfo.useTransformTimeGlobal;
            HL.LustInfo.BUFFTIME = 90;
        }
    }
