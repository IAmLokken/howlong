...
;(function()
{
    ...
    HL.CalculateTime = function(strDuration, fraDuration, lusDuration)
    {
        ...
        let baseTime = (HL.bossHP / HL.playerBaseDPS / 60).toFixed(2);
        ...
        {
            let time = 0;
            let strengthTime = (HL.bossHP / strDPS / 60).toFixed(2);
            if (strengthTime > strDuration)
            {
                let hpRemovedByStrength = (strDPS * 60 * strDuration).toFixed();
                let remainingHP = HL.bossHP - hpRemovedByStrength;
                time = ((hpRemovedByStrength / strDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
        ...
        {
            let time = 0;
            let frailtyTime = (HL.bossHP / fraDPS / 60).toFixed(2);
            if (frailtyTime > fraDuration)
            {
                let hpRemovedByFrailty = (fraDPS * 60 * fraDuration).toFixed();
                let remainingHP = HL.bossHP - hpRemovedByFrailty;
                time = ((hpRemovedByFrailty / fraDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
        ...
        {
            let time = 0;
            let lustTime = (HL.bossHP / lusDPS / 60).toFixed(2);
            if (lustTime > lusDuration)
            {
                let hpRemovedByLust = (lusDPS * 60 * lusDuration).toFixed();
                let remainingHP = HL.bossHP - hpRemovedByLust;
                time = ((hpRemovedByLust / lusDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
        ...
        {
            let time = 0;
            let strengthFrailtyTime = (HL.bossHP / strfraDPS / 60).toFixed(2);

            if (strengthFrailtyTime > fraDuration || strengthFrailtyTime > strDuration)
            {
                if (fraDuration > strDuration)
                {
                    ...
                    let hpRemovedbyFrailty = (fraDPS * 60 * fraDuration - strDuration).toFixed();
                    let remainingHP = fraHP - hpRemovedbyFrailty;
                    if (remainingHP > 0)
                    {
                        time = ((hpRemovedByStrengthFrailty / strfraDPS / 60) + (fraHP / fraDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    }
                    else
                    {
                        time = ((hpRemovedByStrengthFrailty / strfraDPS / 60) + (remainingHP / fraDPS / 60)).toFixed(2);
                    }
                ...
                if(fraDuration < strDuration)
                {
                    let hpRemovedByStrengthFrailty = (strfraDPS * 60 * fraDuration).toFixed();
                    let strHP = HL.bossHP - hpRemovedByStrengthFrailty;
                    let hpRemovedbyStrength = (strDPS * 60 * strDuration - fraDuration).toFixed();
                    let remainingHP = strHP - hpRemovedbyStrength;
                    if (remainingHP > 0)
                    {
                        time = ((hpRemovedByStrengthFrailty / strfraDPS / 60) + (strHP / strDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    }
                    else
                    {
                        time = ((hpRemovedByStrengthFrailty / strfraDPS / 60) + (remainingHP / strDPS / 60)).toFixed(2);
                    }
                    console.log("Time to kill on strength (" + strDuration + ") and frailty (" + fraDuration + "): " + time);
                    return time;
                }
                if (fraDuration == strDuration)
                {
                    ...
                    time = ((hpRemovedByStrengthFrailty / strfraDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    console.log("Time to kill on strength (" + strDuration + ") and frailty (" + fraDuration + "): " + time);
                    return time;
                }
            } 
        ...
        {
            let time = 0;
            let strengthLustTime = (HL.bossHP / strlusDPS / 60).toFixed(2);

            if (strengthLustTime > lusDuration || strengthLustTime > strDuration)
            {
                if (lusDuration > strDuration)
                {
                    ...
                    let hpRemovedbyLust = (lusDPS * 60 * lusDuration - strDuration).toFixed();
                    let remainingHP = lusHP - hpRemovedbyLust;
                    if (remainingHP > 0)
                    {
                        time = ((hpRemovedByStrengthLust / strlusDPS / 60) + (lusHP / lusDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    }
                    else
                    {
                        time = ((hpRemovedByStrengthLust / strlusDPS / 60) + (remainingHP / lusDPS / 60)).toFixed(2);
                    }
                ...
                if(lusDuration < strDuration)
                {
                    let hpRemovedByStrengthLust = (strlusDPS * 60 * lusDuration).toFixed();
                    let strHP = HL.bossHP - hpRemovedByStrengthLust;
                    let hpRemovedbyStrength = (strDPS * 60 * strDuration - lusDuration).toFixed();
                    let remainingHP = strHP - hpRemovedbyStrength;
                    if (remainingHP > 0)
                    {
                        time = ((hpRemovedByStrengthLust / strlusDPS / 60) + (strHP / strDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    }
                    else
                    {
                        time = ((hpRemovedByStrengthLust / strlusDPS / 60) + (remainingHP / strDPS / 60)).toFixed(2);
                    }
                    console.log("Time to kill on strength (" + strDuration + ") and lust (" + lusDuration + "): " + time);
                    return time;
                }
                if (lusDuration == strDuration)
                {
                    ...
                    time = ((hpRemovedByStrengthLust / strlusDPS / 60) + (remainingHP / HL.playerBaseDPS / 60)).toFixed(2);
                    console.log("Time to kill on strength (" + strDuration + ") and lust (" + lusDuration + "): " + time);
                    return time;
                }
            } 
    ...
    HL.JobSelected = function(index, value)
    {
        ...
        for (var i = 0; i < bossList.length; i++)
        {
            if(bossList[i].HP)
            {
                ...
                x.value = bossList[i].HP;
                bossDropdown.add(x, i);
            }
        }

        dpstext.value = value;
        HL.playerBaseDPS = value;
    ...
    HL.BossSelected = function(index, value)
    {
        ...
        HL.bossHP = value;
        HL.OutputTimeToKill(HL.CalculateTime(0,0,0));
    }
