...
;(function()
{
    ...
    HL.GetDamageDone = function(totalTime, strTime, fraTime, lusTime, lusTransformTime, AOE, multiplier)
    {
        var hpRemoved = 0;
        ...
        else if (strTime > 0 && lusTransformTime > 0)
        {
            if (strTime >= lusTransformTime)
            {
                ...
                return totalTime * (HL.lustDPS * 1.3); 
            }
            console.log("Strength active for part of the lust transformation");
            hpRemoved += strTime * (HL.lustDPS * 1.3);
            hpRemoved += (lusTransformTime - strTime) * (HL.lustDPS);
